...
def run(ind, coords):
    ...
    mbws = msel.search(multi_bw_min=[2])
    m = MGWR(coords, y, X, msel, kernel="bisquare", fixed=False).fit()
    sig = m.filter_tvals() != 0              # corrected for multiple testing (da Silva & Fotheringham)
    try:
        mlocal = np.asarray(m.localR2).ravel()
        mlocal_src = "MGWR"
    ...
    return dict(
        ...
        mgwr=dict(R2=r3(m.R2), adjR2=r3(m.adj_R2), AICc=r3(m.aicc), bws=[int(b) for b in mbws]),
        paper=PAPER_TABLE5, localR2_model=mlocal_src,
        coef=[[r3(v) for v in row] for row in m.params],
